...
RadialWave::RadialWave(std::string name, Set2D *effectSet, float centerX, float centerY, float speed) : Effect(std::move(name),speed) {
  this->effectSet = effectSet;
  this->centerX = centerX;
  this->centerY = centerY;
}

void RadialWave::renderEffect(vector<uint32_t> &output) {
