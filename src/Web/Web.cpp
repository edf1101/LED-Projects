...
<head>
  ...
  <style>
    ...
    .slider-container {margin-top: 20px; text-align: left; width: 100%%; max-width: 400px; margin-left: auto; margin-right: auto;}
    .slider-container label {font-size: 1.5rem; display: block; margin-bottom: 10px;}
    .speed-slider {width: 100%%; height: 16px; appearance: none; background: #ddd; border-radius: 10px; outline: none; cursor: pointer;}
    .speed-slider::-webkit-slider-thumb {appearance: none; width: 20px; height: 20px; background: #4CAF50; border-radius: 50%%; cursor: pointer; transition: background 0.2s;}
    .speed-slider::-webkit-slider-thumb:hover {background: #45a049;}
    .speed-slider::-moz-range-thumb {width: 20px; height: 20px; background: #4CAF50; border-radius: 50%%; cursor: pointer; transition: background 0.2s;}
    .speed-slider::-moz-range-thumb:hover {background: #45a049;}
  </style>
</head>
<body>
  ...
  <div class="slider-container">
    <label for="speedSlider">Effect Speed</label>
    <input type="range" id="speedSlider" class="speed-slider" min="0.5" max="1.5" step="0.1" value="1.0" onchange="setSpeed(this.value)">
  </div>
  ...
  <script>
    ...
    function setSpeed(speed) {
      // Send the speed value to ESP32
      var xhr = new XMLHttpRequest();
      xhr.open("GET", `/setSpeed?value=${speed}`, true);
      xhr.send();
    }
  </script>
</body>
...
String Web::processor(const String &var) {
  ...
  if (var == "TITLEPLACEHOLDER") {
    return myProject->getName().c_str();
  }
  return String();
}
...
void Web::configureRoutes() {
  ...
  server.on("/setSpeed", HTTP_GET, [this](AsyncWebServerRequest *request) { handleSetSpeedRequest(request); });
}

void Web::handleSetSpeedRequest(AsyncWebServerRequest *request) {
  if (request->hasParam("value")) {
    String speedValue = request->getParam("value")->value();
    float speed = speedValue.toFloat();
    if (speed >= 0.5f && speed <= 1.5f) {
      myProject->setSpeed(speed); // Assuming `setSpeed` is a method in your `Project` class
//      Serial.println(("Speed updated to " + speedValue).c_str());
